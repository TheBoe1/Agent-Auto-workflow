...
输入主题 / 大方向
        │
        ▼
[Phase 0] 需求澄清闸门：确认账号定位 / 目标受众 / 内容方向
          缺失 → 追问；无账号定位 → 降级「通用内容模式」
          其余两项缺失 → 不得开工
        │ 三项确认（或通用模式 + 受众/方向确认）
        ▼
        建 run 目录，写 00_input.md
        │
        ├───────────────────────────────┐
        ▼                               ▼
...
### Phase 0：需求澄清与输入确认（主理人）
内容生产前必须先确认三项关键输入，缺失即向用户追问，确认前不得开工：
- **账号定位**：目标账号 / 账号类型（如医生垂类、医美、科普……）
- **目标受众**：给谁看（如患者家属、年轻医生、医学生……）
- **内容方向**：本次要表达的核心观点 / 主题切入点

三项确认后建 run 目录、写 `00_input.md`。若用户明确表示没有账号定位，则降级为「通用内容模式」并在 `00_input.md` 头部标注；其余两项缺失时不得开工。
